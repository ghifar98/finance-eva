                ...
                <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                        ...
                        <table class="w-full">
                            <thead class="bg-slate-100 border-b border-slate-200">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-center text-sm font-medium text-slate-700">Rencana Progres</th>
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @php $prevMinggu = null; $prevKode = null; $totalProgres = 0; @endphp
                                    ...
                                    @foreach($items as $wbs)
                                        <tr class="hover:bg-slate-50 transition-colors duration-150">
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                @if($minggu != $prevMinggu || $kode != $prevKode)
                                                    <div class="inline-flex items-center">
                                                        <div class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-medium">
                                                            {{ number_format($wbs->rencana_progres ?? 0, 2) }}%
                                                        </div>
                                                    </div>
                                                    @php $totalProgres += ($wbs->rencana_progres ?? 0); @endphp
                                                @endif
                                            </td>
                    ...
                    <div class="bg-slate-800 px-6 py-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-white">
                            ...
                            <div>
                                <p class="text-sm text-slate-300">Total Unique Tasks</p>
                                <p class="text-xl font-semibold text-blue-400">
                                    {{ count($groupedWbs) }} Task
                                </p>
                            </div>
                            <div>
                                <p class="text-sm text-slate-300">Total Rencana Progres</p>
                                <p class="text-xl font-semibold text-green-400">
                                    {{ number_format($totalProgres, 2) }}%
                                </p>
                            </div>
                        </div>
                        
                        @if($totalProgres > 100)
                            <div class="mt-4 p-3 bg-red-900/50 border border-red-700 rounded-lg">
                                <div class="flex items-center">
                                    <svg class="w-5 h-5 text-red-400 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                                    </svg>
                                    <div>
                                        <p class="text-sm font-medium text-red-200">Peringatan!</p>
                                        <p class="text-sm text-red-300">Total rencana progres melebihi 100%. Harap periksa kembali perhitungan.</p>
                                    </div>
                                </div>
                            </div>
                        @elseif($totalProgres < 100)
                            <div class="mt-4 p-3 bg-yellow-900/50 border border-yellow-700 rounded-lg">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-yellow-200">Informasi</p>
                                        <p class="text-sm text-yellow-300">Sisa {{ number_format(100 - $totalProgres, 2) }}% progres belum dialokasikan.</p>
                                    </div>
                                </div>
                            </div>
                        @else
                            <div class="mt-4 p-3 bg-green-900/50 border border-green-700 rounded-lg">
                                <div class="flex items-center">
                                    <svg class="w-5 h-5 text-green-400 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                    </svg>
                                    <div>
                                        <p class="text-sm font-medium text-green-200">Sempurna!</p>
                                        <p class="text-sm text-green-300">Total rencana progres sudah mencapai 100%.</p>
                                    </div>
                                </div>
                            </div>
                        @endif
